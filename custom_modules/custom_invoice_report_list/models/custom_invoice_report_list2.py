    ...
    def init(self):
        ...
        self.env.cr.execute('''
            CREATE OR REPLACE VIEW %s AS (
                SELECT MIN(a.id) id,
                        a.move_id move_id,
                        MIN(a.product_id) product_id,
                        MIN(a.account_id) account_id,
                        MIN(a.analytic_account_id) analytic_account_id,
                        MIN(a.journal_id) journal_id,
                        MIN(a.company_id) company_id,
                        MIN(a.currency_id) currency_id,
                        MIN(a.partner_id) commercial_partner_id,
                        MIN(a.name) as name,
                        MIN(a.state) state,
                        MIN(a.type) as type,
                        MIN(a.partner_id) partner_id,
                        MIN(a.invoice_user_id) invoice_user_id,
                        MIN(a.fiscal_position_id) fiscal_position_id,
                        MIN(a.invoice_payment_state) invoice_payment_state,
                        MIN(a.invoice_date) invoice_date,
                        MIN(a.invoice_date_due) invoice_date_due,
                        MIN(a.invoice_payment_term_id) invoice_payment_term_id,
                        MIN(a.invoice_partner_bank_id) invoice_partner_bank_id,
                        MIN(a.residual) residual,
                        MIN(a.amount_total) amount_total,
                        MIN(a.product_uom_id) product_uom_id,
                        MIN(a.product_categ_id) product_categ_id,
                        MIN(a.quantity) quantity,
                        MIN(a.price_subtotal) price_subtotal,
                        MIN(a.price_average) price_average,
                        MIN(a.country_id) country_id,
                        MIN(a.nbr_lines) nbr_lines
                FROM (
                %s %s %s %s
                ) as a
                GROUP BY move_id 
